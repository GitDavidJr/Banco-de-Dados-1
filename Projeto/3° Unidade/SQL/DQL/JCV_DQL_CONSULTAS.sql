-- faturamento total
select sum(pedido.valor_total) as faturamento from pedido;

-- custo de mao de obra
select sum(custo_mao_de_obra) as 'custo mao de obra' from produz;

-- custo materia prima
select sum(valor_unit) as 'custo materia prima' from materia_prima;
...
select avg(pedido.valor_total) as 'media de preco dos pedidos' from pedido;
